...
public class SortOptionTest {
    ...
    private static class TestAlignment implements Alignment {
        ...
        TestAlignment(int start, int readOffset, String bases, BaseModificationSet baseModificationSet) {
            byte[] baseBytes = bases.getBytes();
            byte[] readBases = new byte[readOffset + baseBytes.length];
            System.arraycopy(baseBytes, 0, readBases, readOffset, baseBytes.length);
            this.blocks = new AlignmentBlock[]{new AlignmentBlockImpl(start, readBases, null, readOffset, bases.length(), 'M')};
